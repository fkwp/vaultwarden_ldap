...
|`ldap_search_group_of_names_filter`|String|Optional|Filter used when searching LDAP GroupOfNames. Eg. `(&(objectClass=GroupOfNames)(member=*))`. When defined a LDAP search is carried out for each member of the GroupOfNames. Defaults to `` (empty String)|
